...
def words_from_text(text, start, end, raw_words=None):
    """Rebuild word-level timing for a corrected segment.

    Whisper's word list is the karaoke-caption skeleton: ``word`` carries a
    leading space on true word starts, and ``start``/``end`` drive the active
    word highlight. A corrected segment needs words too, but its new text no
    longer lines up with the raw tokens. Corrections are usually 1:1 (a misheard
    word swapped, punctuation fixed), so when the corrected text has the same
    number of words as the raw segment we keep Whisper's exact timings and only
    swap the text; otherwise we distribute the segment span proportionally to
    word length so the highlight still tracks roughly with the audio.
    """
    start = float(start)
    end = float(end)
    ...
    raw = list(raw_words or [])
    if raw and len(raw) == len(tokens):
        out = []
        for tok, rw in zip(tokens, raw):
            try:
                s = float(rw.get("start", start))
                e = float(rw.get("end", end))
            except (TypeError, ValueError, AttributeError):
                s, e = start, end
...
def rebuild_corrected_transcript(corrected_segments, raw_transcript):
    """Merge Gemini-corrected segment TEXT back onto the raw transcript's timing.

    Timing is never trusted from the model: every corrected segment keeps the
    raw segment's start/end, and word timings are re-derived from the raw words
    where possible. Returns the raw transcript unchanged when the model's output
    is missing, the wrong length, or otherwise unusable, so callers always get a
    captionable transcript.
    """
    raw_segments = list((raw_transcript or {}).get("segments", []) or [])
    if not raw_segments:
        return raw_transcript
    corrected = list(corrected_segments or [])
    if len(corrected) != len(raw_segments):
        return raw_transcript

    out_segments = []
    any_change = False
    for raw_seg, cor_seg in zip(raw_segments, corrected):
        raw_text = str(raw_seg.get("text") or "").strip()
        if isinstance(cor_seg, dict):
            new_text = str(cor_seg.get("text") or "").strip()
        else:
            new_text = str(cor_seg or "").strip()
        if not new_text:
            new_text = raw_text
        ...
        out_segments.append({
            ...
            "end": end,
            "text": new_text,
            "words": words_from_text(new_text, start, end, raw_seg.get("words")),
        })

    if not any_change:
        return raw_transcript
    return {
        "text": " ".join(s["text"] for s in out_segments if s["text"]),
