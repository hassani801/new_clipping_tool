...
from clip_selection import (build_transcript_windows, clip_count_targets,
                            clip_duration_bounds, rebuild_corrected_transcript,
...
# --- per-clip phase timing (diagnostic) ------------------------------------
# Clip workers run in parallel (CLIP_WORKERS), so the current clip label is
# thread-local. analyze_scenes_strategy and the caption pass read it to tag
# their [TIMING] lines with the right clip number. Each phase keys its own
# start time so nested phases (scene_analysis runs INSIDE reframe_render) don't
# overwrite each other's clock.
_timing_ctx = threading.local()
_timing_totals_lock = threading.Lock()
_timing_totals = {}  # phase -> cumulative seconds (job-wide)


def _set_timing_clip(label):
    _timing_ctx.clip = label


def _timing_clip():
    return getattr(_timing_ctx, "clip", "clip_?")


def _timing_start(phase):
    starts = getattr(_timing_ctx, "starts", None)
    if starts is None:
        starts = {}
        _timing_ctx.starts = starts
    starts[phase] = time.time()
    print(f"[TIMING] {_timing_clip()}: {phase} started", flush=True)


def _timing_end(phase):
    starts = getattr(_timing_ctx, "starts", {})
    start = starts.pop(phase, time.time())
    elapsed = time.time() - start
    with _timing_totals_lock:
        _timing_totals[phase] = _timing_totals.get(phase, 0.0) + elapsed
    print(f"[TIMING] {_timing_clip()}: {phase} finished ({elapsed:.1f}s)", flush=True)


def _print_timing_summary(total_wall_s):
    """Print a stage-by-stage summary for before/after comparisons."""
    with _timing_totals_lock:
        rows = sorted(_timing_totals.items(), key=lambda kv: kv[1], reverse=True)
    print("\n========== STAGE TIMING SUMMARY ==========", flush=True)
    if not rows:
        print("(no timed stages recorded)", flush=True)
    else:
        for phase, secs in rows:
            print(f"  {phase:28s} {secs:8.1f}s", flush=True)
    print(f"  {'TOTAL_WALL':28s} {total_wall_s:8.1f}s", flush=True)
    print("==========================================\n", flush=True)
...
def transcribe_video(video_path):
    ...
    _set_timing_clip("job")
    _timing_start("transcribe")
    try:
        transcript = transcribe_media(video_path)
    finally:
        _timing_end("transcribe")
...
def _correct_and_score(client, model_name, transcript_result, windows,
                       video_duration, language, costs):
    """One Gemini call that corrects the raw transcript AND scores the windows.

    Returns ``(corrected_transcript, scored)``. ``corrected_transcript`` is
    always a usable transcript (it equals the raw transcript when the model made
    no correction); ``scored`` is None when the call failed or was blocked, in
    which case the caller falls back to the batched score pass on the raw
    transcript. The call's cost is appended to ``costs`` on success.
    """
    ...
    windows_json = json.dumps(
        [{"id": w["id"], "start": w["start"], "end": w["end"], "text": w["text"]}
         for w in windows],
        ensure_ascii=False)
    prompt = gemini_worker.CORRECT_AND_SCORE_PROMPT_TEMPLATE.format(
        video_duration=video_duration, language=language,
        segments_json=segments_json, windows_json=windows_json)
    try:
        parsed, cost = _run_gemini_stage(
            client, model_name, prompt, gemini_worker.CorrectAndScoreResponse)
    except gemini_worker.GeminiBlockedError as e:
        print(f"   🚫 Transcript correction blocked by policy ({e}) — "
              f"scoring the raw transcript instead.")
        return transcript_result, None
    except Exception as e:
        print(f"   ⚠️ Transcript correction failed ({type(e).__name__}: {e}) — "
              f"scoring the raw transcript instead.")
        return transcript_result, None
    if cost:
        costs.append(cost)
    corrected = rebuild_corrected_transcript(
        parsed.get("corrected_transcript"), transcript_result)
    scored = list(parsed.get("windows") or [])
    if corrected is not transcript_result:
        print(f"   ✏️ Corrected transcript via Gemini "
              f"({len(corrected.get('segments', []))} segments).")
...
def get_viral_clips(transcript_result, video_duration):
    ...
    """
    print("\U0001f916  Analyzing with Gemini (correct+score → detail)...")
    ...
    try:
        ...
        # --- Pass 1: correct the transcript AND score windows in ONE call ---
        corrected_transcript, scored = _correct_and_score(
            client, model_name, transcript_result, windows,
            video_duration, language, costs)
        if scored is None:
            # Correction call failed/blocked: fall back to the batched score
            # pass on the raw transcript (which also bisects policy blocks).
            scored = []
            SCORE_BATCH = 8
        ...
        # more candidates without exploding the detail call. Window text is
        # re-derived from the CORRECTED transcript so moment detection sees the
        # cleaned-up words, not the raw ASR output.
        if corrected_transcript is not transcript_result:
            detail_windows = build_transcript_windows(
                corrected_transcript, video_duration,
                window_seconds=max(90, int(max_secs * 1.5)))
        else:
            detail_windows = windows
        scored.sort(key=lambda w: w.get("score", 0), reverse=True)
        target = max(3, min(10, int(video_duration // 90) + 2))
        by_id = {w["id"]: w for w in detail_windows}
        shortlist = [by_id[w["id"]] for w in scored[:target] if w.get("id") in by_id]
        if not shortlist:
            shortlist = detail_windows[:target]  # scoring returned nothing usable
        ...
        if costs:
            ...
            print(f"\U0001f4b0 Total cost ({model_name}, {len(costs)} calls): ${cost_analysis['total_cost']:.6f}")

        if not shorts:
            print("⚠️ Gemini analysis returned no clips.")
            return None

        result = {"shorts": shorts, "corrected_transcript": corrected_transcript}
        if cost_analysis:
...
if __name__ == '__main__':
    ...
    if args.skip_analysis:
        ...
        if not clips_data or 'shorts' not in clips_data:
            ...
            # which is correct (there's no speech to caption). The corrected
            # transcript is what captions and the editor use downstream; the
            # raw faster-whisper output is kept separately for comparison.
            corrected_transcript = clips_data.pop('corrected_transcript', None) or transcript
            clips_data['transcript'] = corrected_transcript or {"language": "none", "segments": []}
            if transcript is not None:
                clips_data['raw_transcript'] = transcript
            ...
            def _process_one_clip(i, clip):
                ...
                try:
                    ...
                    _timing_start("cut_encode")
                    subprocess.run(cut_command, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
                    _timing_end("cut_encode")

                    _timing_start("reframe_render")
                    success = render_clip(clip_temp_path, clip_final_path, output_format)
                    _timing_end("reframe_render")
                    ...
                    if success:
                        _timing_start("caption_burn")
                        captioned = auto_caption_clip(
                            deliver_path, corrected_transcript, start, end,
                            split_ranges=_layouts.split_ranges(clip['layout_ranges']))
                        _timing_end("caption_burn")
    ...
    _print_timing_summary(total_time)
