"""Transcription backends: NVIDIA Parakeet (onnx-asr) plus the swappable
TranscriptProvider implementations (see transcript_providers.py).
...
The whisper backend is one of the transcript_providers implementations,
picked manually via TRANSCRIPT_PROVIDER ("faster_whisper" (default) |
"deepgram"). The parakeet path falls back to the faster-whisper provider
automatically when the model errors, produces no usable words, or the
detected language is outside its 25 supported European languages (e.g.
Japanese/Chinese/Arabic uploads). GPU whisper in turn falls back to CPU
whisper on CUDA errors (VRAM is shared with other models on the host, so
loads can OOM under load).
"""
import os
import subprocess
import tempfile
...
from subtitles import (
    get_whisper_config,
    get_whisper_transcribe_params,
    merge_continuation_words,
)
from transcript_providers import _detect_language, get_transcript_provider
...
class _TranscribeProgress:
    ...
    them), so they must stay free of technical detail. on_progress, when
    given, receives the same honest percent as a float (0.0-100.0) — the
    TranscriptProvider interface hook.
    """

    def __init__(self, total_seconds, on_progress=None):
    ...
    def update(self, position_seconds):
        ...
        while pct >= self.next_pct and self.next_pct <= 100:
            ...
            if self.on_progress:
                self.on_progress(float(self.next_pct))
            self.next_pct += 25
...
def _get_whisper_model():
    ...
    with _whisper_lock:
        if _whisper_model is None or _whisper_key != key:
            ...
            _whisper_key = key
            print(
                f"🎙️ [ASR] WhisperModel loaded "
                f"model={key[0]} device={key[1]} compute={key[2]} "
                f"cpu_threads={key[3]} num_workers={key[4]}",
                flush=True,
            )
    return _whisper_model, cfg["device"]


def _run_whisper_once(media_path, on_progress=None, **params):
    ...
    with gate:
        ...
        progress = _TranscribeProgress(getattr(info, "duration", 0), on_progress)
        materialized = []
...
def run_whisper_transcription(media_path, on_progress=None, **params):
    ...
    try:
        return _run_whisper_once(media_path, on_progress=on_progress, **params)
...
def _transcribe_with_whisper(media_path, on_progress=None):
    params = get_whisper_transcribe_params()
    print(
        f"🎙️ [ASR] faster_whisper decode params: "
        f"beam_size={params['beam_size']} vad_filter={params['vad_filter']} "
        f"word_timestamps={params['word_timestamps']}",
        flush=True,
    )
    segments, info = run_whisper_transcription(
        media_path, on_progress=on_progress, **params)
...
def transcribe_media(media_path, on_progress=None):
    """Transcribe with the configured backend (see module docstring).

    on_progress, when given, receives percent floats (0.0-100.0) from the
    selected provider in addition to the stdout 🎙️ markers.
    """
    ...
    return provider.transcribe(media_path, on_progress=on_progress)
