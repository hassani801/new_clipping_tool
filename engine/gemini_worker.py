...
class CorrectedSegmentModel(BaseModel):
    start: float
    end: float
    text: str


class CorrectAndScoreResponse(BaseModel):
    corrected_transcript: List[CorrectedSegmentModel]
    windows: List[ScoredWindowModel]
...
# One call that does both jobs at once: correct the raw ASR transcript AND score
# the candidate windows, so a smaller/faster local Whisper model can keep its
# speed while Gemini cleans up its minor errors — without a second, separate
# correction API call. The corrected text then drives BOTH moment detection and
# burned-in captions.
CORRECT_AND_SCORE_PROMPT_TEMPLATE = """
You will receive a raw, timestamped transcript generated by an automatic speech
recognition (ASR) system, plus a list of candidate windows to score.

This transcript may contain minor errors: misheard words, wrong punctuation,
garbled names, or unclear phrases, since it was generated by a smaller/faster
ASR model for speed.

STEP 1 — Correct the transcript:
- Fix obvious ASR errors (misheard homophones, garbled words, missing
  punctuation) using context from the surrounding sentences.
- Do NOT invent or guess content that isn't plausibly there — if a phrase is
  truly unclear or nonsensical even with context, leave it as-is or mark it
  with [unclear] rather than fabricating a confident-sounding but potentially
  wrong correction.
- Preserve the original timestamps EXACTLY as given — only correct the TEXT of
  each segment, never the timing.
- Return exactly one corrected segment per input segment, in the SAME order,
  with the same start and end values.

STEP 2 — Score the candidate windows:
- Using the CORRECTED transcript from Step 1, score each candidate window for
  viral potential.
- `score` must be an integer from 0 to 100.
- THE 2-SECOND TEST is the main criterion: would the first 2 seconds of this
  window force a cold viewer (no context) to keep watching? Windows that only
  work with prior context score low.
- Prefer windows with strong hooks, conflict, surprise, outrage, emotion,
  novelty, big numbers, or a clear payoff.
- Ignore weak filler, housekeeping, outros, rambling transitions, and
  low-signal padding unless there is an obvious hook or payoff.
- Echo each window's id, start and end exactly as given.
...
{{
  "corrected_transcript": [
    {{"start": <number>, "end": <number>, "text": "<corrected text>"}}
  ],
  "windows": [
    {{
      "id": "<window id>",
