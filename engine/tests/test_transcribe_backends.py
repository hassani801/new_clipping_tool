...
def test_whisper_model_is_singleton(fake_faster_whisper, monkeypatch):
    ...
    assert fake_faster_whisper[0].cpu_threads == (os.cpu_count() or 1)
    assert fake_faster_whisper[0].num_workers == 1
    assert fake_faster_whisper[0].model_size == "base"
...
def test_whisper_cpu_threads_env_override(fake_faster_whisper, monkeypatch):
    monkeypatch.setenv("WHISPER_CPU_THREADS", "4")
    monkeypatch.setenv("WHISPER_NUM_WORKERS", "1")
    model, _ = tb._get_whisper_model()
    assert model.cpu_threads == 4
    assert model.num_workers == 1


def test_whisper_beam_size_env_default(monkeypatch):
    from subtitles import get_whisper_transcribe_params
    monkeypatch.delenv("WHISPER_BEAM_SIZE", raising=False)
    assert get_whisper_transcribe_params()["beam_size"] == 2
    monkeypatch.setenv("WHISPER_BEAM_SIZE", "5")
    assert get_whisper_transcribe_params()["beam_size"] == 5
