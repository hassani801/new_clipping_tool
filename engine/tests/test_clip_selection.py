...
class TestWordsFromText:
    def test_1_to_1_swap_keeps_raw_timing(self):
        words = words_from_text(
            " cars", 2.0, 4.0, [{"word": " cats", "start": 2.0, "end": 4.0}])
        assert words == [{"word": " cars", "start": 2.0, "end": 4.0}]

    def test_word_count_mismatch_distributes_proportionally(self):
        words = words_from_text(" cars are great", 2.0, 4.0,
                                [{"word": " cats", "start": 2.0, "end": 4.0}])
        assert len(words) == 3
    ...
    def test_empty_text_returns_no_words(self):
        assert words_from_text("   ", 0.0, 1.0) == []


class TestRebuildCorrectedTranscript:
    def _raw(self):
        return {
            "text": "I love cats",
            "language": "en",
            "segments": [
                {"start": 0.0, "end": 2.0, "text": "I love",
                 "words": [{"word": " I", "start": 0.0, "end": 0.5},
                           {"word": " love", "start": 0.5, "end": 2.0}]},
                {"start": 2.0, "end": 4.0, "text": "cats",
                 "words": [{"word": " cats", "start": 2.0, "end": 4.0}]},
            ],
        }

    def test_correction_updates_text_keeps_timing(self):
        raw = self._raw()
        corrected = rebuild_corrected_transcript(
            [{"start": 0.0, "end": 2.0, "text": "I love"},
             {"start": 2.0, "end": 4.0, "text": "cars"}], raw)
        assert corrected["segments"][1]["text"] == "cars"
        assert corrected["segments"][1]["words"][0]["start"] == 2.0
        assert corrected["segments"][1]["words"][0]["word"] == " cars"
        assert corrected["segments"][0]["start"] == 0.0
        assert corrected["segments"][0]["end"] == 2.0
        assert corrected["text"] == "I love cars"

    def test_wrong_length_falls_back_to_raw(self):
        raw = self._raw()
        assert rebuild_corrected_transcript(
            [{"start": 0.0, "end": 4.0, "text": "I love cars"}], raw) is raw

    def test_no_change_returns_raw(self):
        raw = self._raw()
        corrected = rebuild_corrected_transcript(
            [{"start": 0.0, "end": 2.0, "text": "I love"},
             {"start": 2.0, "end": 4.0, "text": "cats"}], raw)
        assert corrected is raw

    def test_empty_corrected_text_falls_back_per_segment(self):
        raw = self._raw()
        corrected = rebuild_corrected_transcript(
            [{"start": 0.0, "end": 2.0, "text": ""},
             {"start": 2.0, "end": 4.0, "text": "cars"}], raw)
        assert corrected["segments"][0]["text"] == "I love"
        assert corrected["segments"][1]["text"] == "cars"

    def test_empty_raw_segments_return_raw(self):
        raw = {"text": "", "language": "en", "segments": []}
        assert rebuild_corrected_transcript([{"start": 0.0, "end": 1.0, "text": "x"}], raw) is raw
