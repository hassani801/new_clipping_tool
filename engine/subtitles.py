...
# main.transcribe_video) behave identically.
# Free-tier default is "base" (faster on CPU than "small"; still caption-usable).
# Paid tier uses Deepgram via TRANSCRIPT_PROVIDER and never hits this default.
# Override anytime with WHISPER_MODEL=small|medium|large-v3-turbo etc.
...
def get_whisper_config():
    ...
    try:
        cpu_threads = int(os.environ.get("WHISPER_CPU_THREADS", str(cpu_count)))
    except ValueError:
    ...
    return {
        ...
        # Pin CTranslate2 threads to available cores (CX32 = 4 vCPU).
        "cpu_threads": max(1, cpu_threads),
        # One ASR job at a time per process — do not oversubscribe workers.
        "num_workers": max(1, num_workers),
    }


def get_whisper_transcribe_params():
    """Decode params for faster-whisper.transcribe(), env-overridable.

    Free-tier default beam_size=2 (was hardcoded 5) — much faster on CPU with
    acceptable caption quality for short-form. A/B via WHISPER_BEAM_SIZE.
    """
    try:
        beam_size = int(os.environ.get("WHISPER_BEAM_SIZE", str(DEFAULT_WHISPER_BEAM_SIZE)))
    except ValueError:
...
# Prefer get_whisper_transcribe_params() so env changes are picked up live.
WHISPER_TRANSCRIBE_PARAMS = get_whisper_transcribe_params()
